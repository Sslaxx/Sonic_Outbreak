...
func go_to_scene (path):
	...
	if (OS.is_debug_build ()):	# FOR DEBUGGING ONLY.
		printerr ("Changing from ", current_scene, " to ", new_scene, ".")
...
"""
   ...
   Try and avoid using this overly in _process functions, but shouldn't make too much of an adverse impact on performance.
"""
